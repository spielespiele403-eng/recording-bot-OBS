"""LoRA fine-tunes the SDXL inpainting UNet on the user's own images.
...
placeholder-token caption is used. Training reconstructs the full image
(no masking) so the LoRA learns the user's visual style for later outpainting.
"""
import argparse
import glob
import os
...
def main(data_dir: str, output_dir: str, epochs: int = 20):
    pipe = AutoPipelineForInpainting.from_pretrained(BASE_MODEL_DIR, torch_dtype=torch.float32)
    ...
    # no-mask training: 0 everywhere means "keep the whole image" during reconstruction
    empty_mask = torch.zeros(1, 1, RESOLUTION // 8, RESOLUTION // 8)
    ...
    for epoch in range(epochs):
        for pixel_values, caption in examples:
            ...
            unet_input = torch.cat([noisy_latents, empty_mask, latents], dim=1)
    ...
    AutoPipelineForInpainting.save_lora_weights(
